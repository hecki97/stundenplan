  ...
  use \Colors\RandomColor;
	...
	**/
    ...
    private static function initialize() {
      ...
      $uuid = UUID::v5(self::$db_data->uuid, USERNAME);
      self::$data_dir_path = PROJECT_DIR.'/resources/data/'.$uuid;
      $file_data = @file_get_contents(self::$data_dir_path.'/data');
      self::$data = (/*!empty(self::$db_data->unique_filename) && */file_exists(self::$data_dir_path.'/data')) ? CryptHandler::Decrypt(self::$db_data->encryption_key, $file_data) : array();
    ...
    //!-- Outdated
      ...
      $data_dir_path = PROJECT_DIR.'/resources/data/'.UUID::v5($db_data->uuid, USERNAME);
      (!is_dir(self::$data_dir_path.'/tables')) ? mkdir(self::$data_dir_path.'/tables') : false;

      // Only for testing!
      $fp = fopen(self::$data_dir_path.'/tables/'.self::$id.'_output', 'w');
      fwrite($fp, print_r($savedata_array, true));
      fclose($fp);
      //

      $fp = fopen(self::$data_dir_path.'/tables/'.self::$id, 'w');
